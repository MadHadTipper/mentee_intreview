"""Level 5a — API smoke check (NOT correctness).

Confirms that ``solution.mdp.load_mdp`` accepts a YAML/dict config that
includes a ``delay: K`` field on an observation term, and that calling the
returned ``obs_fn`` doesn't crash. Does NOT verify the buffer's lag-by-K
semantics or per-env reset behavior — those live in the interviewer's grader.
"""
...
CFG_WITH_DELAY = {
    "observations": [{"name": "joint_pos", "delay": 2}],
...
def test_load_mdp_accepts_delay_field_without_crashing() -> None:
    _, obs_fn, _, obs_dim, _ = load_mdp(CFG_WITH_DELAY)
    out = obs_fn(_state())
